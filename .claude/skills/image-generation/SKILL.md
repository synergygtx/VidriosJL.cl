...
description: |
  ...
  Requiere OPENROUTER_API_KEY configurada en .env.local.
...
> Requiere: `OPENROUTER_API_KEY` en `.env.local`. Ver `.claude/ai_templates/_index.md` para setup del AI engine.
...
# Usando modelo Pro
npx tsx .claude/skills/image-generation/scripts/generate-image.ts \
  --prompt "Detailed architectural blueprint of a software platform" \
