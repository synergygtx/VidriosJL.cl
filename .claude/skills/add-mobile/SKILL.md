...
> **Forge Tip:** Usa el skill `image-generation` para generar iconos PWA programaticamente:
> `npx tsx .claude/skills/image-generation/scripts/generate-image.ts --prompt "App icon for [tu app], minimal, solid background" --aspect 1:1 --output public/icons/icon-512.png`
...
> **Forge Note:** El layout.tsx de Forge usa route groups (auth) y (main). PWARegister debe ir en el ROOT layout (`src/app/layout.tsx`), no en los layouts de los route groups, para que funcione en todas las rutas.
